...
namespace TUGraz.VectoCore.Tests.Integration.EngineOnlyCycle
{
	...
	public class EngineOnlyCycleTest
	{
		...
		[TestMethod]
		public void TestEngineOnlyDrivingCycleVarDt()
		{
			var data = DrivingCycleData.ReadFromFileEngineOnly(@"TestData\Cycles\Coach Engine Only Paux_var-dt.vdri");
			var expectedResults = ModalResults.ReadFromFile(@"TestData\Results\EngineOnlyCycles\24tCoach_EngineOnlyPaux.vmod");
...
//				TestContext.DataRow["CycleFile"].ToString(), "test2.csv");
			...
			var currentCyclePos = data.Entries.GetEnumerator();
			var nextCyclePos = data.Entries.GetEnumerator();
			currentCyclePos.MoveNext();
			nextCyclePos.MoveNext();
			nextCyclePos.MoveNext();
			
			do {
				// foreach (var cycle in data.Entries) {
				var dt = TimeSpan.FromSeconds((nextCyclePos.Current.Time - currentCyclePos.Current.Time).Double());

				port.Request(absTime, dt, currentCyclePos.Current.EngineTorque, currentCyclePos.Current.EngineSpeed);
...
//						if (!Double.IsNaN(dataWriter.GetDouble(field)))
						Assert.AreEqual((double) row[field.GetName()] * siFactor[j], dataWriter.GetDouble(field), tolerances[j],
							String.Format("t: {0}  field: {1}", absTime.TotalSeconds, field));
					}
					if (row[ModalResultField.FC.GetName()] is double) {
						...
						Assert.IsTrue(Double.IsNaN(dataWriter.GetDouble(ModalResultField.FC)), String.Format("t: {0}", absTime.TotalSeconds));
					}
				}

				dataWriter.CommitSimulationStep(absTime, dt);
				absTime += dt;
			} while (currentCyclePos.MoveNext() && nextCyclePos.MoveNext());
